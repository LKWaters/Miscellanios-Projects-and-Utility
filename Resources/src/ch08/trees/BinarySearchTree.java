             ...
             implements BSTInterface<T>
{
  ...
  public int countLess(BinarySearchTree<Golfer> tree, Golfer maxValue){

      int answer = 0;
      int treeSize = tree.size();
      
      for (int x = 1; x <= treeSize; x++){
  ...
  // counts the leafs in a binary tree
  public int countLeaf(BSTNode<T> start){
	  // if the tree is empty return null
	  if(start == null){
		  return 0;
	  }
	  // if both nodes are null return 1
	  else if(start.left == null && start.right == null){
		  return 1;
	  }
	  // if the other two cases dont follow through then recurively check the next two nodes
	  else{
		  return countLeaf(start.left) +  countLeaf(start.right);
	  }
  }
  
  // checks how many nodes only have one child node
  public int singleParent(BSTNode start){
	  // if the binary tree is empty return 0
	  if(start == null){
		  return 0;
	  }
	  // if the child nodes consist of one null node and one node with something in it add one and continue recursion
	  else if((start.left == null && !(start.right == null)) || (!(start.left == null) && start.right == null)){
		  return 1 + singleParent(start.left) +  singleParent(start.right);
	  }
	  // runs if both nodes have something in them and continues to check for single child nodes
	  else{
		  return singleParent(start.left) +  singleParent(start.right);
	  }
  }
  
    // compares two tree
    public boolean similarTree(BSTNode<T> comp1, BSTNode<T> comp2){
	  // if both end in null then return true
	  if (comp1 == null && comp2 == null){
		  return true;
	  }
	  // if they are equal then recursively check their children
	  else if(comp1 == comp2){
		  return similarTree(comp1.right, comp2.right) && similarTree(comp1.left, comp2.left);
	  }
	  // if neither of these are met return false
	  else{
		  return false;
	  }
	 
	  // designed so if its false anywhere in the equation then it will be false
  }
  ...
  public Golfer min(BinarySearchTree<Golfer> tree){
	  BinarySearchTree<Golfer> test = (BinarySearchTree<Golfer>) tree;
	  inOrder((BSTNode<T>) test.root);
	  return (Golfer) root.getInfo();
  }
  
public Golfer max(BinarySearchTree<Golfer> tree){
	BinarySearchTree<Golfer> test = (BinarySearchTree<Golfer>) tree;
	  inOrder((BSTNode<T>) test.root);
	  return (Golfer) getPredecessor(root);
  }
