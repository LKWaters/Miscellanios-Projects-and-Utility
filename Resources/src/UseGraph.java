...
public static int numComponents(WeightedGraphInterface<String> inpu){
	
	// creates an object that refers to the starting vertex
	Object start = inpu.getUnmarked();
	
	//makes a list and adds the vertex to it
	UnboundedQueueInterface<Object> list = new LinkedUnbndQueue<Object>();
	list.enqueue(start);
	
	//if start == null (there is no more unmarked vertex in the graph) then return 0
	if(start == null){
		return 0;
	}
	else{
		
		// makes a int number to return to the user, has to be -1 because it increments on the last time after checking for more vertices
		int number  = -1;
		
		// while the list is not empty this will run
		while(!(list.isEmpty())){
			
			// creates a new linked queue 
			UnboundedQueueInterface<String> var = new LinkedUnbndQueue<String>();
			
			// dequeues list to get a vertex and marks it
			Object temp = list.dequeue();
			inpu.markVertex((String) temp);
			
			// gets the vertices of the temp variable if any and enqueues them onto list if they are not already marked
			var = inpu.getToVertices((String) temp);
			while(!(var.isEmpty())){
				String free = var.dequeue();
				if(!inpu.isMarked(free)){
					list.enqueue(free);
				}
			}
			
			// increases the count of number by 1 for each node visited
			number++;
		}
		
		// returns the number of vertex in this graph then checks for another unmarked one recursively
		return number + numComponents(inpu);
	}
