...
public class EcgCalc {
    ...
    public boolean calculateEcg() {
        ...
        ecgLog.println(
                "Finished calculating ECG " +
                        "table data.\n");

        return (RetValue);
    }
    public void print(){
        for (int i=0; i < ecgResultNumRows;i++){
            System.out.println
                    (ecgResultTime[i]+"," +
                            ""+ecgResultVoltage[i]);
        }
    ...
    private double[]
            ti =
            new double[6];  /* ti converted in radians             */
    private double[]
            ai =
            new double[6];  /* new calculated a                    */
    private double[]
            bi =
            new double[6];  /* new calculated b                    */

    private int
            mstate =
            3;               /*  System state space dimension       */
    private double
            xinitial =
            1.0;        /*  Initial x co-ordinate value        */
    private double
            yinitial =
            0.0;        /*  Initial y co-ordinate value        */
    private double
            zinitial =
            0.04;       /*  Initial z co-ordinate value        */
    private long rseed;
    private double h;
    private double[] rrpc;
     ...
     * FFT
     */
    private void ifft(double[] data, long nn,
                      int isign) {
        ...
        while (n > mmax) {
            ...
            for (m = 1; m < mmax; m += 2) {
                for (i = m; i <= n; i += istep) {
                    j = i + mmax;
                    tempr =
                            wr * data[(int) j] - wi * data[(int) j + 1];
                    tempi =
                            wr * data[(int) j + 1] + wi * data[(int) j];
    ...
    /*--------------------------------------------------------------------------*/
    private void derivspqrst(double t0,
                             double x[],
                             double dxdt[]) {
        ...
        y0 = 0.0;
        a0 =
                1.0 - Math.sqrt((x[1] - x0) * (x[1] - x0) + (x[2] - y0) * (x[2] - y0)) / r0;
        ...
        zbase =
                0.005 * Math.sin(2.0 * PI * paramOb.getFHi() * t0);
        ...
        for (i = 1; i <= k; i++) {
            ...
            dt2 = dt * dt;
            dxdt[3] +=
                    -ai[i] * dt * Math.exp(-0.5 * dt2 / (bi[i] * bi[i]));
        }
     ...
     * GENERATE RR PROCESS
     */
    private void rrprocess(double[] rr,
                           double flo,
                           double fhi,
                           double flostd,
                           double fhistd,
                           ...
                           int n) {
        int i;
        double c1, c2, w1, w2, sig1, sig2, rrmean, rrstd,
                xstd, ratio;
        double df;//,dw1,dw2;
        ...
        for (i = 1; i <= n; i++) {
            ...
            //dw2 = w[i]-w2;
            Hw[i] =
                    (sig1 * Math.exp(-0.5 * (Math.pow(w[i] - w1,
                            2) / Math.pow(c1,
                            2))) / Math.sqrt(2 * PI * c1 * c1)) + (sig2 * Math
                            .exp(-0.5 * (Math.pow(w[i] - w2,
                                    2) / Math.pow(c2,
                                    2))) / Math.sqrt(2 * PI * c2 * c2));
        }
     ...
     * DETECT PEAKS
     */
    private void detectpeaks(double[] ipeak,
                             double[] x,
                             double[] y,
                             double[] z,
                             int n) {
    ...
    private boolean dorun() {
        ...
        q =
                (int) Math.rint(paramOb.getSf() / paramOb.getSfEcg());
        ...
        ecgLog.println("Approximate number of heart beats: " + paramOb
                .getN());
        ecgLog.println("ECG sampling frequency: " + paramOb.getSfEcg() + " Hertz");
        ecgLog.println("Internal sampling frequency: " + paramOb
                .getSf() + " Hertz");
        ecgLog.println(
                "Amplitude of additive uniformly distributed noise: " + paramOb
                        .getANoise() + " mV");
        ...
        Nrr =
                (int) Math.pow(2.0,
                        Math.ceil(Math.log(paramOb.getN() * rrmean * paramOb
                                .getSf()) / Math.log(2.0)));

        ecgLog.println("Using " + Nrr + " = 2^ " + (int) (Math
                .log(1.0 * Nrr) / Math.log(2.0)) + " samples for calculating RR intervals");

        /* create rrprocess with required spectrum */
        double[] rr = new double[Nrr + 1];
        rrprocess(rr,
                paramOb.getFLo(),
                paramOb.getFHi(),
                paramOb.getFLoStd(),
                paramOb.getFHiStd(),
                paramOb.getLfHfRatio(),
                paramOb.getHrMean(),
                paramOb.getHrStd(),
                paramOb.getSf(),
                Nrr);
        ...
        for (i = 1; i <= Nts; i++) {
            zts[i] +=
                    paramOb.getANoise() * (2.0 * ran1() - 1.0);
        }
